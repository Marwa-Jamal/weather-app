...
async function USgetweather(){
    if (navigator.geolocation) {
        navigator.geolocation.getCurrentPosition(async function showPosition(position) {
          ...
          var response = await fetch(`http://api.weatherapi.com/v1/forecast.json?key=9c2416cdc6d44c5ea7a95013250105&q=${coords.lat},${coords.lon}&days=3`);
                            ...
                            <div class="inner">
                                <div class="card bg-black text-white p-3">
                                    ...
                                    <div class="content">
                                        ...
                                        <i>${data.current.condition.icon}</i>
...
// oninput search function one way
  async function searchCoun(){
    if (countryName.value.length==4){
        ...
        console.log(country);
        var response = await fetch(`http://api.weatherapi.com/v1/search.json?key=9c2416cdc6d44c5ea7a95013250105&q=${country}`);
        var countryData = await response.json();
        var response = await fetch(`http://api.weatherapi.com/v1/forecast.json?key=9c2416cdc6d44c5ea7a95013250105&q=${countryData[0].lat},${countryData[0].lon}&days=3`);
        var data = await response.json();
        console.log(data)
